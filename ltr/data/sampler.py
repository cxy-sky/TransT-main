...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while not enough_visible_frames:
            ...
            if dataset.name in ['RGBT234']:
                visible = seq_info_dict['visible']['visible']
            else:
        ...
        if is_video_dataset:
            ...
            # 如果是图像数据集，只需重复图像以生成合成视频 In case of image dataset, just repeat the image to generate synthetic video
        ...
        if dataset.name in ['RGBT234']:
            ...
            data = TensorDict({'template_images': template_frames['visible'],
                               'template_anno': template_anno['bbox']['visible'],
                               'search_images': search_frames['visible'],
                               'search_anno': search_anno['bbox']['visible'],

                               'template_images_T': template_frames['infrared'],
                               'template_anno_T': template_anno['bbox']['infrared'],
                               'search_images_T': search_frames['infrared'],
                               'search_anno_T': search_anno['bbox']['infrared'],

                               'grouth_anno': search_anno['bbox']['grouth']
                               })
        else:
            template_frames, template_anno, meta_obj_template = dataset.get_frames(seq_id, template_frame_ids,
                                                                                   seq_info_dict)
