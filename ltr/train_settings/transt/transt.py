...
def run(settings):
    ...
    # Train datasets
    # 加载数据，在data_specs中后缀是lasot_train_split
    # lasot_train = Lasot(settings.env.lasot_dir, split='train')
    rgbt234_train = Rgbt234(settings.env.rgbt234_dir, split='train')

    # got10k_train = Got10k(settings.env.got10k_dir, split='vottrain')
    # trackingnet_train = TrackingNet(settings.env.trackingnet_dir, set_ids=list(range(4)))
    # coco_train = MSCOCOSeq(settings.env.coco_dir)

    #  联合增强变换，联合应用于对 The joint augmentation transform, that is applied to the pairs jointly
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05))

    # 应用于训练集的增强变换（分别应用于对中的每个图像） The augmentation transform applied to the training set (individually to each image in the pair)
    transform_train = tfm.Transform(tfm.ToTensorAndJitter(0.2),
                                    tfm.Normalize(mean=settings.normalize_mean, std=settings.normalize_std))

    # 要对训练对进行的数据处理 Data processing to do on the training pairs
    ...
    dataset_train = sampler.TransTSampler([rgbt234_train], [1],
                                          samples_per_epoch=1000 * settings.batch_size, max_gap=100,
                                          processing=data_processing_train)
    ...
    # debugging时把fail_safe设置为False
    trainer.train(10, load_latest=True, fail_safe=False)
