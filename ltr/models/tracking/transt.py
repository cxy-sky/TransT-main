...
class TransT(nn.Module):
    ...
    def forward(self, search, template, search_T=None, template_T=None):
        ...
        if search_T  is not None:
            if not isinstance(search_T, NestedTensor):
                search_T = nested_tensor_from_tensor(search_T)
            if not isinstance(template_T, NestedTensor):
                template_T = nested_tensor_from_tensor(template_T)
            feature_search_T, pos_search_T = self.backbone(search_T)
            feature_template_T, pos_template_T = self.backbone(template_T)
            feature_search = feature_search + feature_search_T
            feature_template = feature_template + feature_template_T
